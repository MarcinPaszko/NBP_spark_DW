...
# Ścieżka dostosowana do kontenera Airflow
BASE_PATH = Path("/opt/airflow/data/bronze/incremental_nbp")
BASE_PATH.mkdir(parents=True, exist_ok=True)

# Konfiguracja Sparka z lokalnym Jarem
spark = SparkSession.builder \
    .appName("NBP_Incremental_Airflow") \
...
if api_date <= min_last_date:
    print(f"Dane sa aktualne: {api_date}. Koniec.")
else:
    ...
    try:
        ...
        if final_rates.count() > 0:
            ...
            print(f"Zapisano {final_rates.count()} kursów.")
    except Exception as e: print(f"Info: {e}")
    ...
    except Exception as e: print(f"Info Spready: {e}")

print("NBP Pipeline zakończony.")